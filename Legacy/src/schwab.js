...
export function findOptionForContract(chainData, contract) {
  ...
  // Handle both strike and strikePrice field names (chain-refresh.js uses strikePrice)
  const getStrike = o => Number(o.strike ?? o.strikePrice);
  const target    = Number(contract.strike);
  const exact     = list.find(o => getStrike(o) === target);
  if (exact) return exact;
  return list.reduce((best, o) =>
    Math.abs(getStrike(o) - target) < Math.abs(getStrike(best) - target) ? o : best
  );
}
