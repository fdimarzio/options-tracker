...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "renew") {
      ...
      // Token is from today — call ETrade renew endpoint (returns plain text, not JSON)
      const renewUrl    = `${ETRADE_BASE}/oauth/renew_access_token`;
      ...
      for (const acct of accounts) {
        try {
          ...
          positions.forEach(p => {
            ...
            if (prod?.securityType !== "EQ") return; // equity only
            allPositions.push({
              account:      ACCOUNT_NAMES[String(acct.accountId)] || `ETrade ${String(acct.accountId).slice(-4)}`,
              accountIdKey: acct.accountIdKey,
              symbol:       prod.symbol,
              qty:          p.quantity || 0,
              marketValue:  p.marketValue || 0,
              currentPrice: p.Quick?.lastTrade || null,
              changePct:    p.Quick?.changePct || null,
              changeAmt:    p.Quick?.change || null,
            });
          });
        } catch(e) {
          console.warn(`[etrade positions] ${acct.accountIdKey}:`, e.message);
        }
      }
