// backtest.js
// Pulls 6 months of daily price history from Schwab for all tracked tickers,
// simulates covered call outcomes day by day, and outputs a CSV for analysis.
//
// Usage:
//   node backtest.js
//
...
const TICKERS = ["AAPL","AMZN","AMD","CAT","CEG","COST","GOOG","GOOGL","JPM","LMT","MSFT","NFLX","NVDA","OKLO","TKO","UPS","WDC"];
...
async function run() {
  ...
  for (const ticker of TICKERS) {
    ...
    for (let i = 20; i < candles.length - 7; i++) {
      ...
      const strike    = Math.round(day.close * (1 + OTM_PCT) * 2) / 2; // round to nearest 0.50
      ...
      rows.push({
        ...
        dte:                Math.round((new Date(expiry) - new Date(day.date)) / 86400000),
  ...
  console.log(`\n✓ Written backtest_results.csv — ${rows.length} total rows`);
  ...
  console.log("\nUpload backtest_results.csv to Claude for full analysis.");
}
