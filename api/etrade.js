...
async function etradePost(path, body, method = "POST") {
  ...
  const paramString = Object.keys(oauthParams).sort().map(k => `${pctEncode(k)}=${pctEncode(oauthParams[k])}`).join("&");
  const baseString  = [method, pctEncode(urlBase), pctEncode(paramString)].join("&");
  const signingKey  = `${pctEncode(CONSUMER_SECRET)}&${pctEncode(t.accessTokenSecret)}`;
  ...
  if (!res.ok) throw new Error(`ETrade API ${res.status}: ${data?.Error?.message || text}`);
  return data;
}
  ...
  try {
    ...
    // ── order-preview — ETrade two-step order flow ────────────────────────────
    if (action === "order-preview") {
      const body       = typeof req.body === "string" ? JSON.parse(req.body) : req.body;
      const { contract_id, qty, limit_price, order_type = "LIMIT", duration = "DAY", special_instruction = "NONE" } = body;
      if (!contract_id) return res.status(400).json({ error: "Missing contract_id" });

      // Load contract
      const contractRes = await fetch(`${SUPABASE_URL}/rest/v1/contracts?id=eq.${contract_id}&select=*`, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
      });
      ...
      const existingRes = await fetch(`${SUPABASE_URL}/rest/v1/trade_orders?contract_id=eq.${contract_id}&status=in.(pending_approval,dry_run_approved,submitted)`, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
      });
      const existing = await existingRes.json();
      if (Array.isArray(existing) && existing.length > 0) {
        return res.status(400).json({ error: `A ${existing[0].opt_type} order already exists for this contract (status: ${existing[0].status}). Cancel it first.` });
      }

      // Get live quote for this option
      const quoteSymbol = `${contract.stock}:${new Date(contract.expires).getFullYear().toString().slice(2)}:${String(new Date(contract.expires).getMonth()+1).padStart(2,"0")}:${String(new Date(contract.expires).getDate()).padStart(2,"0")}:${contract.type.toUpperCase()}:${contract.strike}`;
      let livePrice = null;
      try {
        const quoteData = await etradeGet("/v1/market/quote/" + encodeURIComponent(quoteSymbol) + ".json");
        const q = quoteData?.QuoteResponse?.QuoteData?.[0]?.All || quoteData?.QuoteResponse?.QuoteData?.[0];
        if (q) {
          const bid = q.bid ?? q.bidPrice ?? null;
          const ask = q.ask ?? q.askPrice ?? null;
          const mid = bid != null && ask != null ? Math.round(((bid + ask) / 2) * 100) / 100 : null;
          livePrice = { bid, ask, mid };
        }
      } catch (e) {
        console.warn("[etrade order-preview] quote fetch failed:", e.message);
      }

      // Determine closing action
      const closingAction = contract.opt_type === "STO" ? "BUY_CLOSE"
                          : contract.opt_type === "BTO" ? "SELL_CLOSE"
                          : contract.opt_type === "BTC" ? "BUY_CLOSE"
                          : "SELL_CLOSE";

      const orderQty   = qty || contract.qty;
      const limitPrice = limit_price ?? livePrice?.mid ?? null;

      // Get account key for this contract's account
      const acctData = await etradeGet("/v1/accounts/list");
      const accounts = acctData?.AccountListResponse?.Accounts?.Account || [];
      const ACCOUNT_IDS = { "ETrade 6917": "227156917", "ETrade 8222": "227418222" };
      const accountId  = ACCOUNT_IDS[contract.account] || Object.values(ACCOUNT_IDS)[0];
      const acct       = accounts.find(a => String(a.accountId) === String(accountId));
      if (!acct) return res.status(400).json({ error: `Account not found for ${contract.account}` });

      const expires  = new Date(contract.expires);
      const payload  = {
        PreviewOrderRequest: {
          orderType: "OPTN",
          clientOrderId: `app_${Date.now()}`,
          Order: [{
            allOrNone:    special_instruction === "ALL_OR_NONE" ? "true" : "false",
            priceType:    order_type === "MARKET" ? "MARKET" : "LIMIT",
            limitPrice:   order_type !== "MARKET" ? limitPrice : undefined,
            ...
            Instrument: [{
              Product: {
                ...
                expiryYear:   expires.getFullYear(),
                expiryMonth:  expires.getMonth() + 1,
                expiryDay:    expires.getDate(),
                strikePrice:  contract.strike,
              },
              orderAction: closingAction,
      ...
      // Call ETrade preview endpoint
      const previewRes = await etradePost(`/v1/accounts/${acct.accountIdKey}/orders/preview.json`, payload);
      const previewId  = previewRes?.PreviewOrderResponse?.PreviewIds?.[0]?.previewId;
      if (!previewId) {
        console.error("[etrade order-preview] no previewId:", JSON.stringify(previewRes));
        return res.status(500).json({ error: "ETrade preview failed — no previewId returned", details: previewRes });
      }
      ...
      const orderRow = {
        ...
        dry_run:             true,
        raw_request:         { ...payload, previewId, accountIdKey: acct.accountIdKey },
      };

      const saveRes = await fetch(`${SUPABASE_URL}/rest/v1/trade_orders`, {
        method: "POST",
      ...
      return res.status(200).json({
        ...
        preview: {
          action:        `${orderRow.opt_type} ${orderQty} × ${contract.stock} $${contract.strike} ${contract.type} ${contract.expires}`,
    ...
    // ── order-place — approve and submit ETrade order ─────────────────────────
    if (action === "order-place") {
      const body = typeof req.body === "string" ? JSON.parse(req.body) : req.body;
      const { orderId, dry_run, limit_price: overridePrice, order_type, duration, special_instruction } = body;
      if (!orderId) return res.status(400).json({ error: "Missing orderId" });

      const orderRes = await fetch(`${SUPABASE_URL}/rest/v1/trade_orders?id=eq.${orderId}`, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
      });
      const order = (await orderRes.json())?.[0];
      if (!order) return res.status(404).json({ error: "Order not found" });
      if (order.status !== "pending_approval") return res.status(400).json({ error: `Order is ${order.status}, not pending_approval` });
      ...
      if (isDryRun) {
        const patch = { status: "dry_run_approved", dry_run: true, approved_at: new Date().toISOString(), limit_price: order.limit_price, notes: "Dry run — not submitted to ETrade" };
        await fetch(`${SUPABASE_URL}/rest/v1/trade_orders?id=eq.${orderId}`, {
        ...
        return res.status(200).json({ ok: true, dryRun: true, message: "Dry run approved. Set dry_run: false to submit." });
      }

      // Real submit — extract previewId and accountIdKey from raw_request
      const previewId    = order.raw_request?.previewId;
      const accountIdKey = order.raw_request?.accountIdKey;
      if (!previewId || !accountIdKey) return res.status(400).json({ error: "Missing previewId or accountIdKey — re-preview the order" });
      ...
      const payload = {
        PlaceOrderRequest: {
          ...
          Order: [{
            ...
            limitPrice:   order.order_type !== "MARKET" ? order.limit_price : undefined,
      ...
      const placeRes   = await etradePost(`/v1/accounts/${accountIdKey}/orders/place.json`, payload);
      const etradeOrderId = placeRes?.PlaceOrderResponse?.OrderIds?.[0]?.orderId;

      if (!etradeOrderId) {
        await fetch(`${SUPABASE_URL}/rest/v1/trade_orders?id=eq.${orderId}`, {
          method: "PATCH", headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, "Content-Type": "application/json" },
          body: JSON.stringify({ status: "error", error_msg: JSON.stringify(placeRes) }),
        });
      ...
      await fetch(`${SUPABASE_URL}/rest/v1/trade_orders?id=eq.${orderId}`, {
        method: "PATCH", headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ status: "submitted", dry_run: false, schwab_order_id: String(etradeOrderId), submitted_at: new Date().toISOString(), limit_price: order.limit_price, raw_request: payload }),
      });

      // Pushover
      notifyReauth(`✅ ETrade Order Submitted: ${order.ticker} ${order.opt_type} ${order.qty}x $${order.strike} ${order.type} ${order.expires} · $${Number(order.limit_price).toFixed(2)}`).catch(()=>{});

      return res.status(200).json({ ok: true, dryRun: false, etradeOrderId });
    }

    // ── order-cancel — cancel ETrade order ───────────────────────────────────
    if (action === "order-cancel") {
      ...
      const orderRes = await fetch(`${SUPABASE_URL}/rest/v1/trade_orders?id=eq.${orderIdInt}`, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
      });
      const order = (await orderRes.json())?.[0];
      if (!order) return res.status(404).json({ error: "Order not found" });
      ...
      if (["pending_approval","dry_run_approved"].includes(order.status)) {
        await fetch(`${SUPABASE_URL}/rest/v1/trade_orders?id=eq.${orderIdInt}`, {
          method: "PATCH", headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, "Content-Type": "application/json" },
      ...
      if (order.status === "submitted" && order.schwab_order_id) {
        const etradeOrderId = parseInt(order.schwab_order_id, 10);
        const accountIdKey  = order.raw_request?.accountIdKey;
        if (!accountIdKey) return res.status(400).json({ error: "Missing accountIdKey in order" });

        try {
          await etradePost(`/v1/accounts/${accountIdKey}/orders/cancel.json`, {
            CancelOrderRequest: { orderId: etradeOrderId },
          }, "PUT");
        } catch (e) {
          // If already cancelled/rejected, just clean up DB
          console.warn("[etrade order-cancel] cancel call failed:", e.message);
        }
