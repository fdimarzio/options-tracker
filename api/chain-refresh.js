...
export default async function handler(req, res) {
  ...
  try {
    ...
    const cRes = await fetch(`${SUPABASE_URL}/rest/v1/contracts?select=id,stock,type,opt_type,strike,expires,qty&status=eq.Open`, {
      headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
    });
    ...
    const chainData = {};
    for (const { ticker, expiry, key } of pairs) {
      try {
        const r = await fetch(`${SCHWAB_BASE}/marketdata/v1/chains?symbol=${ticker}&contractType=ALL&strikeCount=30&fromDate=${expiry}&toDate=${expiry}`, {
      ...
      } catch (e) { console.warn(`chain failed ${key}:`, e.message); }
    }
    ...
    res.status(200).json({ ok: true, time: lastRefresh, chains: Object.keys(chainData).length });
  } catch (err) {
